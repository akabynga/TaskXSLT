...
<xsl:stylesheet version="1.0"
		...
		<html>
			...
			<body>
				...
				<xsl:for-each select="shop/category[@name=$category]/subcategory">
					<xsl:call-template name="show-subcategory"></xsl:call-template>
				</xsl:for-each>
				<div>
	...
	<xsl:template name="show-subcategory">
		...
		<div>
			<a href="shop?command=showProducts&amp;subcategory={$subcategory}">
				<xsl:value-of select="$subcategory" />
				(
				<xsl:value-of
					select="count(/shop/category[@name=$category]/subcategory[@name=$subcategory]/product)" />
				)
			</a>
		</div>
	</xsl:template>
